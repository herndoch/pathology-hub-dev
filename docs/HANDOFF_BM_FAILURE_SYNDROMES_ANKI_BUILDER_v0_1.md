# Handoff — BM Failure Syndromes Contextual Cloze Anki Builder (v0_1)

Date: 2026-07-13  
Audience: another AI (or human) building the Heme SH **BM Failure Syndromes** contextual-cloze deck  
...
Build an image-grounded, source-aligned contextual cloze Anki deck for **Heme SH BM Failure Syndromes**, following the TNK Lymphomas exemplar and Contextual Cloze SOP v1.1.
...
- `Heme_SH_BM_Failure_Syndromes_package.zip`
...
**Gated chunk warning:** **9** gated chunks. Use as an index only — **not** syllabus/card-scope authority. Always read full transcript, lecture index, and frames.
...
```text
gs://pathology_hub/Heme_SH_BM_Failure_Syndromes_package.zip
```

### Sidecars

```text
gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_bm_failure_syndromes_v0_1/
```
...
```text
heme_sh_bm_failure_syndromes_deck_project/
├── SOP/
...
```bash
BUNDLE=./heme_sh_bm_failure_syndromes_deck_project
...
gsutil cp "gs://pathology_hub/Heme_SH_BM_Failure_Syndromes_package.zip" "$BUNDLE/lecture_1/"

gsutil -m cp \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_bm_failure_syndromes_v0_1/manifest.json" \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_bm_failure_syndromes_v0_1/frames.jsonl" \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_bm_failure_syndromes_v0_1/segments.jsonl" \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_bm_failure_syndromes_v0_1/chunks_indexable.jsonl" \
  "$BUNDLE/lecture_1/"
```
...
```text
Build a contextual-cloze Anki deck for Heme SH BM Failure Syndromes.
...
- chunks_indexable.jsonl — navigation only (9 gated chunk(s) for this series).
