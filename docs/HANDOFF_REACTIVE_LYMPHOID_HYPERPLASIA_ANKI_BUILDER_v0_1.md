# Handoff — Reactive Lymphoid Hyperplasia Contextual Cloze Anki Builder (v0_1)

Date: 2026-07-13  
Audience: another AI (or human) building the Heme SH **Reactive Lymphoid Hyperplasia** contextual-cloze deck  
...
Build an image-grounded, source-aligned contextual cloze Anki deck for **Heme SH Reactive Lymphoid Hyperplasia**, following the TNK Lymphomas exemplar and Contextual Cloze SOP v1.1.
...
Asset library reported 1 missing slide object historically (71/72); verify frame URLs before relying on them.
...
- `Heme_SH_Reactive_Lymphoid_Hyperplasia_package.zip`
...
**Gated chunk warning:** **5** gated chunks. Use as an index only — **not** syllabus/card-scope authority. Always read full transcript, lecture index, and frames.
...
```text
gs://pathology_hub/Heme_SH_Reactive_Lymphoid_Hyperplasia_package.zip
```

### Sidecars

```text
gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_reactive_lymphoid_hyperplasia_v0_1/
```
...
```text
heme_sh_reactive_lymphoid_hyperplasia_deck_project/
├── SOP/
...
```bash
BUNDLE=./heme_sh_reactive_lymphoid_hyperplasia_deck_project
...
gsutil cp "gs://pathology_hub/Heme_SH_Reactive_Lymphoid_Hyperplasia_package.zip" "$BUNDLE/lecture_1/"

gsutil -m cp \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_reactive_lymphoid_hyperplasia_v0_1/manifest.json" \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_reactive_lymphoid_hyperplasia_v0_1/frames.jsonl" \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_reactive_lymphoid_hyperplasia_v0_1/segments.jsonl" \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_reactive_lymphoid_hyperplasia_v0_1/chunks_indexable.jsonl" \
  "$BUNDLE/lecture_1/"
```
...
```text
Build a contextual-cloze Anki deck for Heme SH Reactive Lymphoid Hyperplasia.
...
- chunks_indexable.jsonl — navigation only (5 gated chunk(s) for this series).
