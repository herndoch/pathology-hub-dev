# Handoff — AML Contextual Cloze Anki Builder (v0_1)

Date: 2026-07-13  
Audience: another AI (or human) building the Heme SH **AML** contextual-cloze deck  
...
Build an image-grounded, source-aligned contextual cloze Anki deck for **Heme SH AML**, following the TNK Lymphomas exemplar and Contextual Cloze SOP v1.1.
...
```text
gs://pathology_hub/Heme_SH_AML_package.zip
```

### Sidecars

```text
gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_aml_v0_1/
```
...
```bash
BUNDLE=./heme_sh_aml_deck_project
...
gsutil cp "gs://pathology_hub/Heme_SH_AML_package.zip" "$BUNDLE/lecture_1/"

gsutil -m cp \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_aml_v0_1/manifest.json" \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_aml_v0_1/frames.jsonl" \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_aml_v0_1/segments.jsonl" \
  "gs://pathology_hub/02_normalized/lectures/deck_packages/heme_sh_aml_v0_1/chunks_indexable.jsonl" \
  "$BUNDLE/lecture_1/"
```
...
```text
Build a contextual-cloze Anki deck for Heme SH AML.
