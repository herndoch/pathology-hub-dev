#!/usr/bin/env python3
"""Embed canonical browse leaves for a taxonomy root (Heme, Breast, …).
...
def load_leaves(browse_index: Path, root_label: str) -> list[dict[str, Any]]:
...
def main() -> None:
    ...
    p.add_argument("--root", required=True, help="Browse root label, e.g. Breast or Heme")
    p.add_argument("--out-dir", type=Path, default=None)
    args = p.parse_args()

    root_slug = re_slug(args.root)
    out_dir = args.out_dir or Path(f"outputs/{root_slug}_browse_leaf_embeddings_v0_1")

    leaves = load_leaves(args.browse_index, args.root)
    texts = [leaf_embed_text(leaf) for leaf in leaves]
    client = OpenAI()
    matrix = embed_texts(client, texts)

    out_dir.mkdir(parents=True, exist_ok=True)
    npy_path = out_dir / f"{root_slug}_leaf_embeddings.npy"
    meta_path = out_dir / f"{root_slug}_leaf_embeddings_meta.json"
    np.save(npy_path, matrix)

    meta = {
        "schema_version": "browse_leaf_embeddings.v0_1",
        ...
        "known_limitations": [
            "Leaf embeddings only — segment/window embeddings computed at gate time.",
...
def re_slug(text: str) -> str:
    ...
    return re.sub(r"[^a-zA-Z0-9]+", "_", text.strip()).strip("_").lower() or "root"
