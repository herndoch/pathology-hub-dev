...
# ============================================================
# EVIDENCE SEARCH RELIABILITY v0_2 — SERVER-SIDE INTEGRATION
#
# Feature-flagged wrapper around the existing /evidence/search and
# /health routes. Adds NO new GPT Action / operationId — the external
# contract remains exactly one Action: searchEvidence / POST /evidence/search.
#
# Safety contract:
#   - If EVIDENCE_V0_2_ENABLED is false (default), behavior is 100%
#     byte-identical to the recovered 1.5.10 baseline above.
#   - If v0_2 is enabled but the module fails to import, or query
#     expansion / WHO rerank raises at request time, the baseline
#     response is still returned with an explicit "warnings" entry.
#     No source becomes unavailable solely because v0_2 fails.
#   - Figure/page URL behavior, HTML bundle behavior, and curriculum
#     behavior are fully preserved because this wrapper calls the
#     complete v1.5.10 handler (including its own render_html /
#     curriculum / figure logic) as its baseline step.
#
# Flags (all read from environment at request time so they can be
# toggled per-revision without a rebuild):
#   EVIDENCE_V0_2_ENABLED             (default: false)
#   EVIDENCE_QUERY_EXPANSION_ENABLED  (default: true, only matters if V0_2_ENABLED)
#   EVIDENCE_ROOT_GATING_ENABLED      (default: true, only matters if V0_2_ENABLED)
#   EVIDENCE_WHO_RERANK_ENABLED       (default: true, only matters if V0_2_ENABLED)
#   EVIDENCE_V0_2_DEBUG               (default: false)
# ============================================================

import sys as _sys_v02
import dataclasses as _dc_v02
from pathlib import Path as _Path_v02

_V02_ROOT = _Path_v02(__file__).resolve().parent
if str(_V02_ROOT) not in _sys_v02.path:
    _sys_v02.path.insert(0, str(_V02_ROOT))


def _env_bool_v02(name: str, default: bool) -> bool:
...
EVIDENCE_V0_2_ENABLED = _env_bool_v02("EVIDENCE_V0_2_ENABLED", False)
EVIDENCE_WHO_RERANK_ENABLED_DEFAULT = True
EVIDENCE_V0_2_DEBUG_DEFAULT = False
APP_VERSION_OVERRIDE_V02 = os.environ.get("EVIDENCE_HUB_APP_VERSION_OVERRIDE", "")

_V02_IMPORT_ERROR = None
_v02_load_config = None
_v02_preprocess = None
_v02_rerank_who = None

if EVIDENCE_V0_2_ENABLED:
    try:
        from evidence_search_reliability_v0_2.config import load_config as _v02_load_config
        from evidence_search_reliability_v0_2.integration import (
            preprocess_evidence_search_request as _v02_preprocess,
            rerank_who_results as _v02_rerank_who,
        )
    except Exception as _v02_exc:
        _V02_IMPORT_ERROR = repr(_v02_exc)
        _v02_load_config = None
        _v02_preprocess = None
        _v02_rerank_who = None

_V02_MODULE_LOADED = bool(_v02_load_config and _v02_preprocess and _v02_rerank_who)

# Capture the fully-formed v1.5.10 baseline endpoints (health + search, including HTML
# bundle / curriculum / figure logic) before replacing them one final time.
_BASELINE_HEALTH_ENDPOINT_V02 = None
_BASELINE_SEARCH_ENDPOINT_V02 = None
for _r in list(app.router.routes):
    if getattr(_r, "path", None) == "/health" and "GET" in getattr(_r, "methods", set()):
        _BASELINE_HEALTH_ENDPOINT_V02 = getattr(_r, "endpoint", None)
    if getattr(_r, "path", None) == "/evidence/search" and "POST" in getattr(_r, "methods", set()):
        _BASELINE_SEARCH_ENDPOINT_V02 = getattr(_r, "endpoint", None)
...
def health_v02():
    base = {}
    if _BASELINE_HEALTH_ENDPOINT_V02:
        try:
            base = _BASELINE_HEALTH_ENDPOINT_V02()
        except Exception as e:
            base = {"baseline_health_error": repr(e)}
    if not isinstance(base, dict):
        base = {"baseline_health": str(base)}

    base["evidence_v0_2_enabled"] = bool(EVIDENCE_V0_2_ENABLED)
    base["evidence_v0_2_module_loaded"] = bool(_V02_MODULE_LOADED)
    if _V02_IMPORT_ERROR:
        base["evidence_v0_2_import_error"] = _V02_IMPORT_ERROR
    base["evidence_query_expansion_enabled"] = EVIDENCE_V0_2_ENABLED and _env_bool_v02(
        "EVIDENCE_QUERY_EXPANSION_ENABLED", True
    )
    base["evidence_root_gating_enabled"] = EVIDENCE_V0_2_ENABLED and _env_bool_v02(
        "EVIDENCE_ROOT_GATING_ENABLED", True
    )
    base["evidence_who_rerank_enabled"] = EVIDENCE_V0_2_ENABLED and _env_bool_v02(
        "EVIDENCE_WHO_RERANK_ENABLED", EVIDENCE_WHO_RERANK_ENABLED_DEFAULT
    )
    base["evidence_v0_2_debug"] = _env_bool_v02("EVIDENCE_V0_2_DEBUG", EVIDENCE_V0_2_DEBUG_DEFAULT)
    if APP_VERSION_OVERRIDE_V02:
        base["version"] = APP_VERSION_OVERRIDE_V02
    return base


@app.post("/evidence/search")
def search_evidence_v02(
    ...
    if not _BASELINE_SEARCH_ENDPOINT_V02:
        raise HTTPException(status_code=500, detail="Baseline searchEvidence endpoint not found for v0_2 wrapper.")

    def _call_baseline(request_model):
        try:
            return _BASELINE_SEARCH_ENDPOINT_V02(request_model, request, x_api_key)
        except TypeError:
            return _BASELINE_SEARCH_ENDPOINT_V02(req=request_model, request=request, x_api_key=x_api_key)

    if not EVIDENCE_V0_2_ENABLED:
        return _call_baseline(req)

    if not _V02_MODULE_LOADED:
        resp = _call_baseline(req)
        if isinstance(resp, dict):
            resp.setdefault("warnings", []).append(
                f"v0_2_unavailable_baseline_used: {_V02_IMPORT_ERROR or 'module not loaded'}"
            )
        if APP_VERSION_OVERRIDE_V02 and isinstance(resp, dict):
            resp["version"] = APP_VERSION_OVERRIDE_V02
        return resp
    ...
    try:
        v02_cfg = _v02_load_config(
            enabled=_env_bool_v02("EVIDENCE_QUERY_EXPANSION_ENABLED", True),
            debug=_env_bool_v02("EVIDENCE_V0_2_DEBUG", EVIDENCE_V0_2_DEBUG_DEFAULT),
            root_gating_enabled=_env_bool_v02("EVIDENCE_ROOT_GATING_ENABLED", True),
        )
        payload = req.dict()
        new_payload, expansion, diagnostics = _v02_preprocess(payload, config=v02_cfg)
        if new_payload.get("query") and new_payload.get("query") != original_query:
            effective_req = req.copy(update={"query": new_payload["query"]})
    except Exception as e:
        v02_warning = f"v0_2_query_expansion_failed_using_baseline_query: {repr(e)}"
        effective_req = req
        expansion = None
        diagnostics = None

    try:
        resp = _call_baseline(effective_req)
    except Exception as e:
        # The v0_2-modified query broke dispatch entirely. Retry with the ORIGINAL
        # query so no source becomes unavailable solely because v0_2 changed it.
        resp = _call_baseline(req)
        v02_warning = (v02_warning + "; " if v02_warning else "") + (
            f"v0_2_expanded_query_dispatch_failed_retried_original_query: {repr(e)}"
        )

    if not isinstance(resp, dict):
        return resp

    who_rerank_enabled = _env_bool_v02("EVIDENCE_WHO_RERANK_ENABLED", EVIDENCE_WHO_RERANK_ENABLED_DEFAULT)
    if expansion is not None and who_rerank_enabled:
        try:
            resp = _v02_rerank_who(resp, query=original_query, expansion=expansion)
        except Exception as e:
            v02_warning = (v02_warning + "; " if v02_warning else "") + (
                f"v0_2_who_rerank_failed_baseline_ranking_used: {repr(e)}"
            )

    debug_enabled = _env_bool_v02("EVIDENCE_V0_2_DEBUG", EVIDENCE_V0_2_DEBUG_DEFAULT)
    if diagnostics and debug_enabled:
        resp.setdefault("diagnostics", {}).update(diagnostics)

    if v02_warning:
        resp.setdefault("warnings", []).append(v02_warning)

    if expansion is not None and getattr(expansion, "effective_query", None) and expansion.effective_query != original_query:
        resp["query_expansion_applied"] = True
        if debug_enabled:
            resp["query_original"] = original_query
            resp["query_effective"] = expansion.effective_query
