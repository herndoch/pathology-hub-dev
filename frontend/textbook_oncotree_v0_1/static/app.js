(() => {
  const DATA_URL = "data/textbook_oncotree_index_v0_1.json";
  ...
  function collectItems(node) {
    ...
    return out.filter((it) => {
      const key = it.chunk_id || `${it.source_id}|${it.page}|${it.excerpt?.slice(0, 40)}`;
  ...
  function nodeMatches(node, q) {
    ...
    for (const it of collectItems(node)) {
      const cHay = `${it.source_title || ""} ${it.source_id || ""} ${it.excerpt || ""} ${it.figure_id || ""}`.toLowerCase();
  ...
  function selectNode(node) {
    ...
    detailPath.textContent = `${node.path} · ${node.chunk_count || items.length} chunks in index · showing ${items.length} samples`;
    ...
    items.forEach((it) => {
      ...
      li.className = "clip" + (it.chunk_id === activeItemId ? " active" : "");
      const kind = it.chunk_type === "figure_caption" ? "Figure" : "Text";
      const page = it.page != null ? `p.${it.page}` : "";
      li.innerHTML = `
        <div class="title">${escapeHtml(it.source_title || it.source_id || "Textbook")} · ${kind} ${escapeHtml(page)}</div>
        <div class="meta">${escapeHtml(it.figure_id || it.section || "")}</div>
