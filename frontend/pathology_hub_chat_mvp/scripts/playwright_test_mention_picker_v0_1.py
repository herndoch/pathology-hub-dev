...
was actually wanted):
  ...
  2026-08-04 (v2): dropdown redesigned to render the live OncoTree instead
    ...
    see dropdown menu?").
  2026-08-04 (v3, this version): no separate widget at all — an active
    "@mention" takes over browse-content with the SAME live OncoTree the
    rest of Browse uses (renderMentionSearchInTree), with "+" buttons
    (renderMentionAddButton / mentionMode) on matching leaves instead of
    "VS" ("i still see vs and not plus sign").
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        if browse_content.locator(".oncotree-leaf ~ .vs-btn:not(.mention-add-btn)").count() > 0:
            _fail("a plain VS button (not the + variant) is still showing on a mention-mode leaf", None)
        _ok("matching leaves show '+' buttons, not 'VS'", f"{add_btns.count()} button(s)")

        # --- Click the "+" button: inserts a semicolon-delimited mention ---
        ...
        _ok("clicked + inserted mention into the search bar", val_after_first)
        ...
        # --- Type "@DCIS" and pick by clicking the leaf's label directly (not its +) ---
        ...
        try:
            page.wait_for_selector("text=Compare Diagnoses", timeout=240000)
