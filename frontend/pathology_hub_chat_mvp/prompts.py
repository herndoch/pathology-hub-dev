...
def pathologist_page_review_system_prompt() -> str:
    ...
    return (
        ...
        "4. CALIBRATION (critical — do not fail everything):\n"
        "   - `ready_for_human_review`: use when the draft is substantively grounded in the "
        "bundle, major supported sections are present, figures are mostly in the right "
        "modality sections, and there are NO fatal unsupported claims. Residual polish, "
        "one missing optional section, or a nicer DDx table belong under Nice-to-have — "
        "NOT automatic needs_fixes.\n"
        "   - `needs_fixes`: material problems only — unsupported clinical/histologic claims, "
        "required Key Literature missing despite literature cards, hub textbook/WHO/PathOut "
        "cards unused, or clearly wrong figure modality placement.\n"
        "   - `blocked_thin_evidence`: empty/near-empty cards, or answer is only an error note.\n"
        "   Default toward `ready_for_human_review` when the page would help a practicing "
        "pathologist and evidence is adequate. Aim for a realistic mix, not universal rejection.\n\n"
        "OUTPUT EXACTLY this markdown structure (no preamble):\n"
        "## Verdict\n"
        "- Exactly one of: `ready_for_human_review` OR `needs_fixes` OR `blocked_thin_evidence` "
        "(emit only the chosen token once; do not list the other options).\n"
        "- One short sentence why.\n\n"
        "## Score\n"
        "- Integer 0–100 (publishability given THIS evidence; 75+ usually ready_for_human_review).\n\n"
        "## Strengths\n"
