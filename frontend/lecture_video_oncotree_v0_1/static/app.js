(() => {
  ...
  function collectClips(node) {
    ...
    for (const child of node.children || []) out.push(...collectClips(child));
    const seen = new Set();
    return out.filter((c) => {
  ...
  function filterRoots(roots, q) {
    ...
    function filterNode(node) {
      ...
      // Keep matching branches even if children filtered; if leaf-ish keep clips
  ...
  /** Visual OncoTree layout — same geometry idea as Chat MVP Browse. */
  ...
  function renderToolbar() {
    ...
    document.getElementById("ot-expand")?.addEventListener("click", () => {
      if (expanded.size) {
        ...
        for (const r of index.roots) expanded.add(r.path || r.id);
      }
      render();
    });
  ...
  function renderOncotree(roots) {
    ...
    treeEl.querySelectorAll(".oncotree-node[data-path]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (!src) return;
        const hasKids = (src.children || []).length > 0;
        if (hasKids) {
  ...
  function selectNode(node) {
    ...
    clips.forEach((clip) => {
      ...
      li.className = "clip" + (clip.chunk_id === activeClipId ? " active" : "");
      li.innerHTML = `
  ...
  player.addEventListener("timeupdate", () => {
    if (clipEndSec != null && player.currentTime >= clipEndSec) player.pause();
  });
  ...
  function render() {
    ...
    const roots = filterRoots(index.roots, q);
    renderToolbar();
    renderOncotree(roots);
  }
  ...
  fetch(DATA_URL)
    ...
    .then((data) => {
      ...
      // Start with roots visible (collapsed children) — classic OncoTree home.
      expanded.clear();
      render();
    })
    .catch((err) => {
